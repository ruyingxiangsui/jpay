...
public class CardService extends HostApduService implements JPayEngine {
	...
	private static final byte[] SELECT_APDU = BuildSelectApdu(JPAY_LOYALTY_CARD_AID);
	
	private static final Gson gson = new Gson();
	private static Command tmp = new Command();
	private static Command rcvCmd = new Command();
	private static TransUtil transUtil;
	
	private static WeakReference<AccountCallback> mAccountCallback;

	public interface AccountCallback {
		public void onReceived(String count);
	}
	...
	public byte[] processCommandApdu(byte[] commandApdu, Bundle extras) {
		...
		// send the loyalty card account number, followed by a SELECT_OK status
		// trailer (0x9000).
		if (Arrays.equals(SELECT_APDU, commandApdu)) {
			//返回卡号和UUID
			Log.i("card", "start...");
			String str_cmd = SPUtil.getCurrentUserInfo(this).getUserName();
			String uuid = SPUtil.getCurrentTransUUID(this);
			byte[] cmdBytes = str_cmd.getBytes();
			Log.i("card", "Returning trans cmd: " + ByteArrayToHexString(cmdBytes));
			
			return ConcatArrays(cmdBytes, SELECT_OK_SW);
		}else if(Arrays.equals(SELECT_OK_SW, commandApdu)){
			//B收到A的OK后，B金额增加,将交易记录写入数据库(成功收款),并返回DONE。
			SimpleDateFormat sDateFormat = new SimpleDateFormat(
					"yyyy-MM-dd   hh:mm:ss");
			String time = sDateFormat.format(new java.util.Date());
			TranstionItem item = new TranstionItem();
			item.setTransCount(tmp.getTransCount()+"");
			item.setTransMyCard(SPUtil.getCurrentUserInfo(this).getUserName());
			item.setTransOtherCard(tmp.getHuanKuanCard());
			item.setTransType(TRANS_TYPE_SHOUKUAN);
			item.setTransTime(time);
			transUtil.insertTrans(item);
			SPUtil.clearCurrentTransUUID(this);
			mAccountCallback.get().onReceived(tmp.getTransCount()+"");
			Log.d("G:", "G:CAO");
			return SELECT_DONE_SW;
		}
		else {			
			String strRcv;
			try {
				strRcv = new String(commandApdu, "UTF-8");
				Log.d("G:", "G:"+ByteArrayToHexString(commandApdu));
				rcvCmd = gson.fromJson(strRcv, Command.class);
				if(!transUtil.isExistTrans(rcvCmd)){
					//如果本地没有该记录先缓存该记录则向A 返回DOING
					tmp = rcvCmd;
					return SELECT_DOING_SW;
				}else{
					return SELECT_DONE_SW;
				}
